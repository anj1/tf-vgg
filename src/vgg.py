...
Example usage: vgg16 with input size 224 and no dropout, for inference:
	python vgg.py 224 16 0.0 1000 models/vgg16.meta
or, for instance, vgg19 with input size 64 with dropout, for training:
	python vgg.py 64 19 0.5 1000 models/vgg19.meta
...
def fc_layer(x, in_channels, out_channels, dropout_frac, name):
	with tf.variable_scope(name):
		...
		z = tf.nn.relu(fc)
		if dropout_frac > 0.0:
			z = tf.nn.dropout(z, dropout_frac)
		return z 
...
def vgg(x, num_classes, patch_size=224, vgg_type="16", dropout_frac=0.0):
	if vgg_type == "16":
		nc = 3
	elif vgg_type == "19":
		nc = 4
	...
	with tf.variable_scope("vgg"):
		...
		fc6   = fc_layer(pool5, ((patch_size/(2**5))**2)*512, 4096, dropout_frac, "fc_6")
		fc7   = fc_layer(fc6,   4096, 4096, dropout_frac, "fc_7")
		fc8   = fc_layer(fc7,   4096, num_classes, 0.0, "fc_8")
		return tf.nn.softmax(fc8, name="prob")

if __name__ == "__main__":
	patch_size   =   int(sys.argv[1])
	vgg_type     =       sys.argv[2]
	dropout_frac = float(sys.argv[3])
	num_classes  =   int(sys.argv[4])
	out_filename =       sys.argv[5]
	x = tf.placeholder(shape=[None, patch_size, patch_size, 3], dtype=tf.float32, name="input")
	prob = vgg(x, num_classes, patch_size, vgg_type, dropout_frac)
